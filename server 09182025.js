...
app.post('/appIndex', (req, res) => {
  ...
  var processString = req.url.split('=')[1];
  var processArray = processString.split(',');
  console.log('processArray len: ' + processArray.length);
...
app.post('/showConfig', (req, res) => {
  ...
  for (var i = 0; i < config.panels.length; i++) {
    htmlString +=
      `
    <div class="" style="border: solid black 3px;width: 320px;padding: 20px;border-radius: 10px;margin: 5px">
    <input id="lblPanel` +
      i +
      `" value="` +
      config.panels[i].panel +
      ...
      `" value="` +
      config.panels[i].panel +
      `"class="panel" style="margin-left: 10px;font-weight: 600;background-color: white;color: purple;font-size:20px;width: 100%;border: solid black 0px;text-align: center" contenteditable />      
      <div style="display: flex;margin: auto;justify-content: center">
      <input type="submit" value="Submit" class="btn btn-primary w3-margin" style="width: 100px">         
      <button hidden id="btnUpdate" onclick="updateConfig(` +
      i +
      ",'" +
      config.panels[i].panel.toLowerCase() +
      `\')" type="button" class="btn btn-primary w3-margin ` +
      config.panels[i].panel.toLowerCase() +
    ...
    for (var j = 0; j < config.panels[i].fields.length; j++) {
      ...
      <div style="margin-bottom: 20px">
        ...
        `" value="` +
        config.panels[i].fields[j].label +
        `:" style="display: flex;margin: auto;color: purple;font-size: 20px;font-weight: 700;width: 100%;border: solid black 0px" contenteditable />
      <input id="` +
        config.panels[i].fields[j].id +
        `" name="` +
        config.panels[i].fields[j].id +
        ...
        config.panels[i].fields[j].class +
        `" value="` +
        config.panels[i].fields[j].value.toLowerCase() +
        `" style="color: blue;font-size: 20px;border: solid black 0px;padding-left: 0px" checked>
                      </div>`;
    }
    htmlString += `
                </div>`;
...
app.post('/updateConfig', (req, res) => {
  ...
  //var lblField = document.getElementById('lblPanel6');
  //console.log(lblField.id);
  /*
  console.log((config.panels[6].panel = 'Test'));
  */
  var configFilePath = 'c:/episd/config.json';
  config.panels[6].panel = value;
  /*
  fs.writeFile(
    configFilePath,
