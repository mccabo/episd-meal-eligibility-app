...
app.post('/showConfig', (req, res) => {
    ...
    <script> 		
      ...
      function updateConfig(i) {
        ...
        for (let i = 0; i < elements.length; i++) {
          if(elements[i].id == 'lblPanel6') {
            //alert(elements[i].id); // Access each element
            //alert(elements[i].innerHTML);
            curPanelLabel =elements[i].innerHTML;
            
            //alert('curPanelLabel: '+curPanelLabel); // Access each element
            //elements[i].style.color = 'pink'; // Modify element style
          } else if(elements[i].id == 'btnUpdate') {
            //alert(elements[i].id); // Access each element
                        
            //Create a variable to represent the print button              
            const updateButton = document.getElementById('btnUpdate');
                        
            //alert(updateButton.formAction());
            updateButton.formAction = "http://localhost:3000/updateConfig?panelValue=test";
            //Simulate a click on the button to trigger submission with the new formaction
            updateButton.click();
                        
            const myForm = document.getElementById("myForm");
            myForm.action = "http://localhost:3000/updateConfig?value="+curPanelLabel;
  ...
  for (var i = 0; i < config.panels.length; i++) {
    ...
    <label id="lblPanel` +
      i +
      `" class="panel" style="margin-left: 0px;font-weight: 600;color: purple;font-size:20px;width: 100%;border: solid black 1px;text-align: center" contenteditable>` +
      config.panels[i].panel +
      `</label>                   
    <div style="display: flex;margin: auto;justify-content: center">                
      <input id="btnUpdate" onclick="updateConfig(` +
      i +
      `)" type="button" class="btn btn-primary w3-margin panel" style="width: 100px;height: 35px">
    </div>   
    `;
