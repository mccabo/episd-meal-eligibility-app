...
app.post('/updateConfig', (req, res) => {
  ...
  */
  ...
  //console.log('Number of Panels: ' + config.panels.length);
  //for (var i = 0; i < config.panels.length; i++) {
  //config.panels[i].panel = dataArrray[i].split('=')[1];
  for (var i = 0; i < config.panels.length; i++) {
    //config.panels[i].panel = dataArrray[i].split('=')[1];

    //console.log('dataArrray: ' + dataArrray[i]);
    for (var j = 0; j < dataArrray.length; j++) {
      if (dataArrray[j].indexOf('panelLbl' + i) != -1) {
        config.panels[i].panel = dataArrray[j].split('=')[1];
        //console.log('dataArrray: ' + dataArrray[j]);
      }
    }
  }
  ...
  for (var i = 0; i < config.panels.length; i++) {
    //config.panels[i].panel = dataArrray[i].split('=')[1];
    for (var j = 0; j < config.panels[i].fields.length; j++) {
      //console.log(config.panels[i].fields);
      //console.log(config.panels[i].fields);

      //config.panels[i].fields[i] = dataArrray[i].split('=')[1];

      //console.log('dataArrray: ' + dataArrray[i]);
      for (var k = 0; k < dataArrray.length; k++) {
        var id = dataArrray[k].split('=')[0];
        var val = dataArrray[k].split('=')[1];
        if (id == config.panels[i].fields[j].id) {
          console.log('i: ' + i + j + k + ': id' + ': ' + id);
          console.log('i: ' + i + j + k + ': val' + ': ' + val);
          ...
          //config.panels[i].fields[j].label = dataArrray[k].split('=')[1];
        }
      }
    }
  }

  //console.log('dataArrray[0]: ' + dataArrray[0].split('=')[1]);
  //config.panels[0].panel = dataArrray[0].split('=')[1];
  //config.panels[1].panel = dataArrray[1].split('=')[1];
