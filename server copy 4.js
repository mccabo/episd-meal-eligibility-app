...
app.post('/printxxxx', (req, res) => {
  ...
  //const username = req.body.appid;

  //const app = JSON.stringify(req.url); // '4'
  //console.log('app: ' + app);
  //console.log('app email: ' + app[0].email);
  //console.log('appid: ' + app[0].appid);
  //console.log('appid: ' + JSON.stringify(app[0].appid));
  ...
  //var appIdTD = "txt"
  //var plainTextValue = $("#txtAppId").text();
  //console.log(plainTextValue);

  //console.log('App Id: ' + req.body.txtAppId.html);
  console.log('print');
  var fileName = '';
  var dest = '';
  ...
  for (var i = 0; i < apps.Applications.length; i++) {
    ...
    if (appId == apps.Applications[i].Id) {
      ...
      */
      ...
      const printRequestResponse =
        ...
        <head>
            ...
            <script>
              /*
                ...
                setTimeout(function() {
                //your code to be executed after 1 second
                window.location.replace("http://localhost:3000/email?appid=2025971&email=mvc757@gmail.com&indexString=5");            
                }, delayInMilliseconds);
              */
            </script>
            <style>  
        ...
        <body>
          <label style="display: flex:margin: auto;font-size: 30px;color: purple">
            Your request to print application Id ` +
        appId +
        ` was successfully printed.</label> 
        </body>
        </html>
    `;

      res.write(printRequestResponse);
      res.end();
      ...
      res.write(
        'Application Id ' +
          appId +
          ' was successfully printed and sent to ' +
          email
      );
      res.end();
      */
...
app.get('/emailx', (req, res) => {
  ...
  const emailRequestResponse =
    `
        ...
        <body>
          <label style="display: flex:margin: auto;font-size: 30px;color: purple">
            Your request to email application Id ` +
    appId +
  ...
  res.write(emailRequestResponse);
  res.end();
});
...
app.post('/email', (req, res) => {
  ...
  for (var i = 0; i < apps.Applications.length; i++) {
    if (appid == apps.Applications[i].Id) {
      console.log('i: ' + i);
      pdfIndex = 0; //added 1 because letters is in index 0 in json file
        ...
        `                   
              <div class="w3-card-4 w3-margin" style="width:18%">
                <header class="w3-container" style="color: #0a58ca">
                  <h1>` +
          apps.Applications[i].Id +
          `</h1>
                </header>
                <div class="w3-container">
                  <h1 style="color: purple;font-size: 32px">` +
          apps.Applications[i].Guardians[0].LastName +
          ', ' +
          apps.Applications[i].Guardians[0].FirstName +
          `</h1>      
                </div>
                <footer class="w3-container" style="color: #0a58ca">
                  <h1 style="margin-top: 2px">` +
          apps.Applications[i].Guardians[0].Email +
          `</h1>
                </footer>  
              </div> 
            `
...
app.post('/printbyId', (req, res) => {
  ...
  for (var i = 0; i < appArray.length; i++) {
    ...
    var email = appArray[i].split(':')[1].trim();
    console.log('appId: ' + appId);
    console.log('email: ' + email);
    //console.log('appArray[' + i + ']: ' + appArray[i]);
