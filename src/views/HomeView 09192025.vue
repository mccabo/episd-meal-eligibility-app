<template> 
  ...
  <form action="/default-action" method="POST" style="border: solid black 0px;width: 100%;max-width: none;">
    ...
    <div class="w-full btn" style="display: inline-flex;margin: auto;justify-content: left;color: white;border-radius: 5px;
      margin-bottom: 5px">
      ...
      <div id="divUtilitiesWrapper" class="divUtilitiesWrapper" style="display: flex;margin: auto;justify-content: center;float: auto;flex-direction: row;flex-wrap: wrap;
        border: solid rgb(0, 0, 0) 0px;">     
      <div style="display: flex;margin: auto;justify-content: center;border: solid black 0px" class="">
        <div style="display: flex;margin: auto;border: solid black 0px;justify-content: center;width: 500px">
          <form action="/default-action" method="POST" style="display: flex;margin: auto;border: solid black 0px;">
            <div id="divUtilitiesMain" v-for="(search,i) in searches" :key="i">          
              
              <div v-if="i>10" :id="search.id" class="w-full" :style="search.divMainStyle">{{ search.label }}</div>            
                
              <div v-if="i>10">               
                <div v-for="(input,j) in search.inputs" :key="j" :id="input.id" style="border: solid rgb(0, 0, 0) 0px" class="w3-margin utilities">                                                    
                  <input v-if="i=='11'" :id="input.id" @mouseover="docEvent(11,0)" @click="showPDFS()" :type="input.type" :style="input.style" :class="input.class" :value="input.label" :formaction="input.formaction" />                                                                                
    ...
    <div id="divSearchWrapper" class="divSearchWrapper" style="display: flex;margin: auto;justify-content: center;float: auto;flex-direction: row;flex-wrap: wrap;
      border: solid rgb(0, 0, 0) 0px;">     
    ...
    <div id="divApps" class="divAppWrapper" style="overflow-x: auto;margin: 0px auto;max-height: 350px;min-height: 100px;
      ...
      <table id="tblMain" class="table table-light table-hover">      
        ...
        <tbody v-if="filteredIds.length>0">                          
          <tr v-for="(application,index) in filteredIds" :key="index" @mouseover="setOver(index)" @mouseleave="onLeave()" style="font-size: 20px;height: 50px">            
            ...
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName)" style="font-size: 20px;padding-left: 30px;color: red;font-weight: 700">{{ application.BatchNum }}</td>                                
            <td style="padding-bottom: 15px"><input @click="setActive(index)" :id="'inpActive'+index" name="inpSelect" type="radio" class="w3-radio inpActive"></td>            
            ...
            <td :id="'txtId'+index" class="approw" @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName)" style="font-size: 20px;padding-left: 0px;color: black">
              {{ application.Id }}</td>            
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName)" style="font-size: 20px;padding-left: 0px;color: black">  
            ...
            <td :id="'txtEmail'+index" class="approw" @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName)" style="font-size: 20px;padding-left: 0px;color: black" 
              contenteditable="false">{{ application.Guardians[0].Email }}</td>
            <td @click="onActive(index)" contenteditable="false" v-if="application.Status[0].Checked=='true'" style="color: green;padding-left: 0px">
              {{ application.Status[0].MobileStatus }}</td>          
            <td @click="onActive(index)" contenteditable="false" v-else-if="application.Status[1].Checked=='true'" style="color: green;height: 30px;padding-left: 0px">            
              {{ application.Status[1].MobileStatus }}</td>          
            <td @click="onActive(index)" contenteditable="false" v-else-if="application.Status[2].Checked=='true'" style="color: red;padding-left: 0px">
              {{ application.Status[2].MobileStatus }}</td>
            <td @click="onActive(index)" contenteditable="false" v-if="application.Reasons[0].Checked=='true'" style="padding-left: 0px;color: black">
              {{ application.Reasons[0].MobileReason }}</td>
            <td @click="onActive(index)" contenteditable="false" v-else-if="application.Reasons[1].Checked=='true'" style="padding-left: 0px;color: black">
              {{ application.Reasons[1].MobileReason }}</td>
            <td @click="onActive(index)" contenteditable="false" style="padding-left: 40px;color: black" v-else></td> 
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName)" style="font-size: 20px;padding-left: 0px;color: black;
...
<script>
  ...
  import { Applications } from 'C:/EPISD/applications.json'  
  ...
  export default {
      ...
      methods: {                
        showPDFS() {
          ...
          if(selectedIndexes!='') {
            ...
            setTimeout(function() {
              window.location.href = "http://DESKTOP-DJNVAF5:3000?appIndex="+processArray;
            }, 300)
          }
        },
        importApps() {
    ...
    setup() {
      ...
      const onActive = (i,id,lname,fname) => {
        activeIndex.value = i        
        activeSet.value = 1
      ...
      return { printLetter,onLeave,submitEmail,submitEmails,setOver,setActive,toggleAppInfo,toggleStudentInfo,onActive,searchApps,selectedStatus,status,selectedReason,reasons,
        ...
        toggleEditLetter,toggleSearch,toggleUtilities,utilities,appId,email,appArray,printData,sendData }}}
</script>
<style>
