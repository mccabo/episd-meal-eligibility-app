<template> 
  ...
  <form @submit.prevent="handleSubmit"> -->
  <form @submit.prevent="handleSubmit" action="/default-action" method="POST" style="border: solid black 0px;width: 100%;max-width: none;">
    ...
    <div v-if="user=='Mary Lou'" class="w-full btn" style="display: inline-flex;margin: auto;justify-content: left;color: white;border-radius: 5px;
      margin-bottom: 5px">
      <input id="divUtilities" @click="toggleUtilities()" type="button" class="btn" style="text-align: left;margin-left: 0px;font-size: 18px;font-weight: 600;
        color: white" value="Utilities"/>
    </div>

      <div v-if="user=='Mary Lou'" id="divUtilitiesWrapper" class="divUtilitiesWrapper" style="display: flex;margin: auto;justify-content: center;
      ...
      <div style="display: flex;margin: auto;justify-content: center;border: solid black 0px" class="w-full">
        <div style="display: flex;margin: auto;border: solid black 0px;justify-content: center" class="w-full">
          <form @submit.prevent="handleSubmit" action="/default-action" method="POST" style="display: flex;margin: auto;border: solid black 0px;max-width: none;height:100px;margin-top: 0px">
            <div id="divUtilitiesMain" v-for="(search,i) in searches" :key="i" style="display: block;margin: 0px;border: solid black 0px;margin-top: -5px">          
              ...
              <div v-if="i>10" :id="search.id" class="w-full" style="display: flex;margin: auto;justify-content: center;text-align: center;
                border:solid black 0px;color: #0a58ca;font-size: 18px;font-weight: 700;height: 15px">{{ search.label }}</div>            
                          
              <div v-if="i>10" style="display: flex;margin:auto;border: solid black 0px;width: 120px;height: 75px">               
                <div v-for="(input,j) in search.inputs" :key="j" :id="input.id" class="utilities" style="display: flex;margin:auto;border: solid black 0px;width: 120px">                                                    
                  <button v-if="i=='11'" type="submit">Submit</button>
                  <!-- <input :id="input.id" @mouseover="docEvent(11,0)" type="submit" :style="input.style" :class="input.class" :value="input.label" :formaction="input.formaction" /> -->
    ...
    <div class="w-full btn" style="display: inline-flex;margin: auto;justify-content: left;border: solid black 0px;color: white;border-radius: 5px;
      margin-bottom: 5px">
      <input id="divApplicationInfo" @click="toggleSearch()" type="button" class="btn" style="text-align: left;margin-left: 0px;font-size: 18px;font-weight: 600;
    ...
    <div class="w-full btn" style="display: inline-flex;margin: auto;justify-content: left;border: solid black 0px;color: white;border-radius: 5px;
      margin-bottom: 5px">
      <input id="divApplicationInfo" @click="toggleAppInfo()" type="button" class="btn" style="text-align: left;margin-left: 0px;font-size: 18px;font-weight: 600;
      ...
      <table id="tblMain">              
        <tbody>  
          <tr style="text-align: center">
            <th contenteditable="true" style="border: solid black 0px;width: 100px;color: black;font-size: 18px">{{ selectAll }} All</th>
    ...
    <div id="divApps" class="divAppWrapper" style="overflow-x: auto;margin: 0px auto;max-height: 325px;min-height: 100px;
      ...
      <table id="tblMain" class="table table-light table-hover">      
        <thead>
          <tr style="font-size: 20px">                             
            ...
            <th contenteditable="true" style="color: purple;border: solid black 0px;width: 175px;padding-left: 0px;padding-right: 0px;text-align: center;">Actions</th>          
          </tr>        
        </thead>       
        <tbody v-if="filteredIds.length>0">                          
          <tr v-for="(application,index) in filteredIds" :key="index" @mouseover="setOver(index)" @mouseleave="onLeave()" style="font-size: 20px;height: 50px">            
            ...
            <td style="padding-bottom: 15px"><input v-if="application.Sent == 'true'" id="chkSent" name="inpSent" type="checkbox" class="w3-check sent" checked disabled>
              <input v-else id="chkSent" name="inpSent" type="checkbox" class="w3-check sent" disabled></td>      
            <td style="padding-bottom: 15px"><input v-if="application.Printed == 'true'" id="chkPrinted" name="inpPrinted" type="checkbox" class="w3-check printed" checked disabled>
              <input v-else id="chkPrinted" name="inpPrinted" type="checkbox" class="w3-check printed" disabled></td>        
            <td :id="'txtId'+index" class="approw" @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" style="font-size: 20px;padding-left: 0px;color: black">
              {{ application.Id }}</td>            
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" style="font-size: 20px;padding-left: 0px;color: black">  
              {{ application.Language }}</td>            
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstNameindex,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" style="font-size: 20px;padding-left: 0px;color: black" 
              contenteditable="false">{{ application.Guardians[0].FirstName }}</td>            
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" style="font-size: 20px;padding-left: 0px;color: black" 
              contenteditable="false">{{ application.Guardians[0].LastName }}</td>
            <td :id="'txtEmail'+index" class="approw" @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" style="font-size: 20px;padding-left: 0px;color: black" 
              contenteditable="false">{{ application.Guardians[0].Email }}</td>
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" contenteditable="false" v-if="application.Status[0].Checked=='true'" style="color: green;padding-left: 0px">
              {{ application.Status[0].MobileStatus }}</td>          
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" contenteditable="false" v-else-if="application.Status[1].Checked=='true'" style="color: green;height: 30px;padding-left: 0px">            
              {{ application.Status[1].MobileStatus }}</td>          
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" contenteditable="false" v-else-if="application.Status[2].Checked=='true'" style="color: red;padding-left: 0px">
              {{ application.Status[2].MobileStatus }}</td>
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" contenteditable="false" v-if="application.Reasons[0].Checked=='true'" style="padding-left: 0px;color: black">
              {{ application.Reasons[0].MobileReason }}</td>
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" contenteditable="false" v-else-if="application.Reasons[1].Checked=='true'" style="padding-left: 0px;color: black">
              {{ application.Reasons[1].MobileReason }}</td>
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" contenteditable="false" style="padding-left: 40px;color: black" v-else></td> 
            <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" style="font-size: 20px;padding-left: 0px;color: black;
            ...
            <input @click="printData(index)" :id="'inpPrint'+index" name="inpPrint" type="button" class="w3-btn inpPrint" style="display: flex;margin: auto;text-align: center;
              margin-left: 20px;font-size: 14px;background-color: #0a58ca;color: white" value="Print" disabled></td>
            <td :id="'divEmail'+index" style="display:  inline-flex;margin: auto;border: solid black 0px;justify-content: center">            
            <input @click="submitEmail()" :id="'inpEmail'+index" name="inpEmail" type="button" class="w3-btn inpEmail" style="display: flex;margin: auto;text-align: center;
              margin-left: 0px;font-size: 14px;background-color: #0a58ca;color: white" value="Email" disabled></td></tr>
        </tbody>
    ...
    <div class="w-full btn" style="display: inline-flex;margin: auto;justify-content: left;border: solid black 0px;color: white;border-radius: 5px;
      margin-bottom: 5px">
      <input id="divSearchBy" @click="toggleStudentInfo()" class="btn" type="button" style="text-align: left;font-size: 18px;font-weight: 600;
    ...
    <div v-if="applications.length>0" class="divDocumentationWrapper hidden" id="divDocumentationWrapper" style="display: flex;margin: auto;justify-content: left;
      border: solid rgb(0, 0, 0) 0px;background-color: rgb(255, 255, 255);overflow: auto;margin-top: 5px;border-radius: 10px">    
      <div class="w-full btn" style="display: flex;margin: auto;flex-direction: row;justify-content: center;align-items: center;
        flex-wrap: wrap;border: solid black 0px">
        <input id="divDoc" type="button" class="btn" style="text-align: left;font-size: 18px;font-weight: 600;
    ...
    <div v-if="applications.length>0" id="divDisplay" style="display: flex;margin: auto;justify-content: center;margin-top: 0px;border: solid black 0px;position:fixed;
      ...
      <div id="divDisplay2" style="display: flex;margin: auto;justify-content: center;margin-top: 0px;margin-bottom: 0px;border: solid black 0px" class="w-full">            
        <input id="inpPrint" type="submit" value="Print Selected Letters" @click="printData" :formaction="'http://localhost:3000/print'" class="btn btn-primary w3-margin w-30rem" style="font-size: 24px" />              
        <input id="inpSend" type="submit" value="Send Selected Letters" @click="sendData" :formaction="'http://localhost:3000/email'" class="btn btn-primary w3-margin w-30rem" style="font-size: 24px" />
      </div>            
    </div>   

    <!-- Accordian button for Eligibility Letter Section -->
    <div v-if="user=='Mary Lou'" class="w-full btn" style="display: flex;margin: auto;flex-direction: row;justify-content: center;align-items: center;flex-wrap: wrap;
      border: solid black 0px;margin-top: 5px">
      <input id="divSearchBy" @click="toggleEditLetter()" type="button" class="btn" style="text-align: left;font-size: 18px;font-weight: 600;
        color: white" value="Edit Eligibility Letter"/>                          
    </div>
    <!-- Letter in HTML Format-->
    <div v-if="applications.length>0" id="divEligibilityLetter" style="display: flex;margin: auto;font-size: 24px;border: solid black 0px;padding: 20px;justify-content: left;
      align-items: center;height: 800px" class="divEligibilityLetter hidden">
      <div v-if="applications[activeIndex].Language=='English'" class="" style="display: block;margin: auto;margin-top: 0px;font-size: 24px;border: solid black 1px;padding: 20px;
      ...
      <div v-if="applications[activeIndex].Language=='Spanish'" class="" style="display: block;margin: auto;margin-top: 0px;font-size: 24px;border: solid black 1px;padding: 20px;
        justify-content: center;align-items: center;width: 50%">        
        <div style="display: flex;margin: auto; justify-content: space-between;border: solid black 0px;width: 100%">      
          <div id="divTitle" class="prop" onkeyup="showProps(this.id,this.innerHTML)" onclick="showProps(this.id,this.innerHTML)"
            style="display: flex;margin: auto; justify-content: center;border: solid black 0px;
      ...
      <div class="" style="display: block;margin: auto;margin-top: 0px;font-size: 24px;border: solid black 1px;margin-left: 10px;justify-content: center;align-items: center;width: 50%">
...
<script>
  ...
  import { Applications } from 'D:/EPISD/applications.json'  
  import { Test } from 'D:/Projects/EPISD/deploy template/public/test/applications.json'  
  import { Expected } from 'D:/Projects/EPISD/deploy template/public/test/applications.json'  
    ...
    setup() {
      ...
      const handleSubmit = () => {
        ...
        // Example: Send data to an API
         axios.post('http://localhost:3000/showLetters?selectedIndexes='+selectedIndexes)
      ...
      const printData = () => {
        ...
        printButton.formAction = 'http://localhost:3000/printbyId?appString='+appString;
