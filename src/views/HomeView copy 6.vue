<template> 
  ...
  todos Copy 8 Localhost serverData: {{ serverData }}
  <!-- todos Localhost todos: '{{ JSON.parse(JSON.stringify(todos.Applications)) }}'
  ...
  <form id="frmMain" :action="'http://localhost:3000/appIndex?frmData='+this.selected" method="POST" style="border: solid black 0px;width: 100%;
    ...
    <div id="divAppsSelectAll" class="divAppWrapper" style="overflow-x: auto;margin: auto;
      border: solid black 10px;border-radius: 0px;justify-content: center;margin-bottom: 10px;border-radius: 0px;
        ...
        <table id="tblMain" class="table table-light table-hover">      
          ...
          <tbody v-if="filteredIds.length>0">                          
            <tr v-for="(application,index) in filteredIds" :key="index" @mouseover="setOver(index)" @mouseleave="onLeave()" style="font-size: 20px;height: 50px">            
              ...
              <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,application.BatchNum)" style="font-size: 20px;padding-left: 30px;color: red;font-weight: 700"><input :value ="application.BatchNum" style="width: 200px"></td>                                
              ...
              <td :id="'divEmail'+index" 
                style="display:  inline-flex;margin: auto;border: solid black 0px;justify-content: center">            
              <input :id="'inpUpdate'+index" name="inpUpdateRow" type="button" class="w3-btn inpUpdateRow" 
                style="display: flex;margin: auto;text-align: center;margin-left: 0px;font-size: 18px;background-color: #F1F5F9;
                color: black;width: 100px;border: solid black 1px" value="Update Row" disabled></td>                
                </tr>
          </tbody>
  ...
  export default {
    ...
    name: "apps",
      mounted: function() {
        this.isMobile()               
        this.id = uniqueId()
        axios.get("http://localhost/applications.json").then((response) => {
          this.serverData = response.data;
          alert("1: "+this.serverData);
        })
      },    
      methods: {                        
        importApps() {
      ...
      data() {
        return {
          appData: {
            url: "http://localhost/applications.json",
            todos: [],
            serverData: []
          },
          url: "http://localhost/applications.json",
    ...
    setup() {
      ...
      return { printLetter,onLeave,submitEmail,submitEmails,setOver,setActive,toggleAppInfo,toggleStudentInfo,onActive,searchApps,selectedStatus,status,selectedReason,reasons,
        ...
        toggleEditLetter,toggleSearch,toggleUtilities,utilities,appId,email,appArray,printData,sendData,addSite,handleSubmit,obj }}}
</script>
<style>
