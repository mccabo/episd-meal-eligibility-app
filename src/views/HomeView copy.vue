<template> 
  {{ searchtype }}
  <form action="http://DESKTOP-DJNVAF5:3600?appIndex=0" method="POST" class="hidden">
    ...
    <div style="display:flex;margin: auto;justify-content: center;">
      <button type="submit" class="w3-margin">Log in</button>
    </div>
  </form>
      ...
      <div style="display: flex;margin: auto;border: solid black 0px;justify-content: left;">
        <div id="divSearchMain" v-for="(search,i) in searches" :key="i" :style="search.divMainStyle">          
          ...
          <div :class="search.class" :style="search.divSearchStyle" >
            <form id="frmProcess" :action="'http://localhost:3000?'+submitTo" method="POST" style="display: flex;margin: auto;flex-wrap: nowrap;border: solid black 0px; width: 100%">
              <div v-for="(input,j) in searches[i].inputs" :key="j" :id="input.id" style="display: flex;margin: auto;justify-content: left;border: solid rgb(0, 0, 0) 0px" 
                class="search hidden">                          
                <input v-if="i!=6" @mouseover="onOver(i,j)" @click="onClick(input.searchType)" @keyup="onClick(input.searchType)" :id="input.id" 
                  :type="input.type" :style="input.style" :class="input.class" :value="input.label" :placeholder="input.placeholder" :model="search.model"/>                                            
                <Dropdown v-else v-model="selectedStatus" @mouseover="docEvent(6,0)"  :options="status" filter optionLabel="status" placeholder="Status">
    ...
    <table id="tblMain" class="table table-light table-hover">      
      <thead>
        <tr style="font-size: 20px">                             
          <th v-for="(appheader,i) in appheaders" :key="i" contenteditable="true" style="color: purple;border: solid black 0px;padding-left: 0px;text-align: center;">{{ appheader }}</th>                    
        </tr>        
      </thead>       
      <tbody v-if="filteredIds.length>0">                          
        <tr v-for="(application,index) in filteredIds" :key="index" @mouseover="setOver(index)" @mouseleave="onLeave()" style="font-size: 20px;height: 50px">            
          ...
          <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName)" style="font-size: 20px;padding-left: 0px;color: black" 
            contenteditable="true">{{ application.Guardians[0].FirstName }}</td>            
          <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName)" style="font-size: 20px;padding-left: 0px;color: black" 
            contenteditable="true">{{ application.Guardians[0].LastName }}</td>
          <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName)" style="font-size: 20px;padding-left: 0px;color: black" 
            contenteditable="true">{{ application.Guardians[0].Email }}</td>
          <td @click="onActive(index)" contenteditable="true" v-if="application.Status[0].Checked=='true'" style="color: green;padding-left: 0px">
            {{ application.Status[0].MobileStatus }}</td>          
          <td @click="onActive(index)" contenteditable="true" v-else-if="application.Status[1].Checked=='true'" style="color: green;height: 30px;padding-left: 0px">            
            {{ application.Status[1].MobileStatus }}</td>          
          <td @click="onActive(index)" contenteditable="true" v-else-if="application.Status[2].Checked=='true'" style="color: red;padding-left: 0px">
            {{ application.Status[2].MobileStatus }}</td>
          <td @click="onActive(index)" contenteditable="true" v-if="application.Reasons[0].Checked=='true'" style="padding-left: 0px;color: black">
            {{ application.Reasons[0].MobileReason }}</td>
          <td @click="onActive(index)" contenteditable="true" v-else-if="application.Reasons[1].Checked=='true'" style="padding-left: 0px;color: black">
            {{ application.Reasons[1].MobileReason }}</td>
          <td @click="onActive(index)" contenteditable="true" style="padding-left: 40px;color: black" v-else></td> 
          <td @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName)" style="font-size: 20px;padding-left: 0px;color: black;
          ...
          <input @click="printLetter(index)" :id="'inpPrint'+index" name="inpPrint" type="button" class="w3-btn inpPrint" style="display: flex;margin: auto;text-align: center;
            margin-left: 0px;font-size: 14px;background-color: #0a58ca;color: white" value="Print" disabled></td>
  ...
  <div v-if="setDocFlag==true" class="divDocumentationWrapper" id="divDocumentationWrapper" style="display: flex;margin: auto;justify-content: left;
...
<script>
  ...
  import { AppHeaders } from '../assets/json/searches.json'
  ...
  export default {
        ...
        filteredIds() {             
          if($("#chkDocumentation").is(':checked')) {
            ...
            if(this.searchtype== 'sentTrue') {          
              ...
              return this.applications.filter(
                (applications) =>
                applications.Status[2].Checked == "true" && applications.Language == "English"
              );            
            } else if(this.searchtype== 'recordid') {
              alert("type: "+this.searchtype)            
              return this.applications.filter(
    ...
    setup() {
      ...
      const appheaders = ref(AppHeaders)    
      const ids = []
      const guardians = []
      const students = []
      const campus = []
      ...
      const importApps = () => {          
          const myForm = document.getElementById('frmProcess');
          myForm.action = 'http://localhost:3000?importApps=1';
          myForm.submit();
          //alert(this.submitTo)
          /*
      ...
      const showConfig = () => {                    
          const myForm = document.getElementById('frmProcess');
          myForm.action = 'http://localhost:3000?showConfig=1';
          myForm.submit();
          //alert(this.submitTo)
          /*
          //window.location.href = "http://localhost:3000?importApps=1";          
      ...
      return { printLetter,onLeave,submitEmail,submitEmails,setOver,setActive,toggleAppInfo,toggleStudentInfo,onActive,searchApps,selectedStatus,status,selectedReason,reasons,
        ...
        toggleEditLetter,toggleSearch,submitTo,importApps,showConfig,appheaders }}}
</script>
<style>
