<template>
    <!-- documentation len: '{{ documentation.keyFeatures.length }}'<br>
customers: {{ customers }}<br><br>
  sites: {{ sites }}
  readonly: {{ readonly }}<br>
  Gold 09302025 4
  activeTab: {{ activeTab }}<br> -->
  <form id="frmMain" :action="action" method="POST" style="border: solid black 0px;width: 100%;
    ...
    <div v-if="user=='System Administrator'" class="" style="display: flex;margin: auto;justify-content: center;border: solid black 0px;width: 100%;margin-top: 0px;margin-bottom: 5px;height: 60px">
      <div v-for="tab in tabs" :key="tab.id">
        <button v-if="tab.id === 'print'"          
          ...
          :class="['w3-button w3-margin hidden', { active: activeTab === tab.id }]" active-class="print"
        style="border: solid black 0px;width: 175px;height: 40px;font-size: 14px;font-weight: 600;color: white;background-color: #0a58ca">
          {{ tab.name }}
        </button>
        <button v-else-if="tab.id === 'show'"          
          @click="activeTab = tab.id"
          :class="['w3-button w3-margin hidden', { active: activeTab === tab.id }]"
        style="border: solid black 0px;width: 175px;height: 40px;font-size: 14px;font-weight: 600;color: white;background-color: #0a58ca">
          {{ tab.name }}
        </button>
        <button v-else-if="tab.id === 'send'"          
          @click="activeTab = tab.id"
          :class="['w3-button w3-margin hidden', { active: activeTab === tab.id }]"
        style="border: solid black 0px;width: 175px;height: 40px;font-size: 14px;font-weight: 600;color: white;background-color: #0a58ca">
          {{ tab.name }}
        </button>
        <button v-else-if="tab.id === 'update'"          
          @click="activeTab = tab.id"
          :class="['w3-button w3-margin hidden', { active: activeTab === tab.id }]"
        style="border: solid black 0px;width: 175px;height: 40px;font-size: 14px;font-weight: 600;color: white;background-color: #0a58ca">
          {{ tab.name }}
        </button>
        <router-link v-else-if="tab.id === 'applications'" :to="{ name: 'Home' }" class="applications-link"
          @click="activeTab = tab.id"
          style="text-decoration: none;color: white;background-color: #a0a8aa;font-size: 14px;font-weight: 600;border-radius: 4px;margin-top: 15px;
          display: flex;justify-content: center;align-items: center;transition: all 0.3s ease;width: 175px;height: 40px;margin-left: 15px"  >
          Applications
        </router-link>                
        
        <router-link v-else-if="tab.id === 'editLetter'"  to="/edit-letters" class="documentation-link"
          style="text-decoration: none;color: white;background-color: #a0a8aa;font-size: 14px;font-weight: 600;border-radius: 4px;margin-top: 15px;
          display: flex;justify-content: center;align-items: center;transition: all 0.3s ease;width: 175px;height: 40px;margin-left: 15px"  >
          Edit Letters</router-link>   

        <router-link v-else-if="tab.id === 'documentation'" :to="{ name: 'Documentation' }" class="documentation-link"
          style="text-decoration: none;color: white;background-color: #a0a8aa;font-size: 14px;font-weight: 600;border-radius: 4px;margin-top: 15px;
          display: flex;justify-content: center;align-items: center;transition: all 0.3s ease;width: 175px;height: 40px;margin-left: 15px"  >
          Documentation
        </router-link>        
        <router-link v-else-if="tab.id === 'todo'" :to="{ name: 'Todo' }" class="todo-link"
          style="text-decoration: none;color: white;background-color: #f59e0b;font-size: 14px;font-weight: 600;border-radius: 4px;margin-top: 15px;
          display: flex;justify-content: center;align-items: center;transition: all 0.3s ease;width: 175px;height: 40px;margin-left: 30px"  >
          Todo List
        </router-link>
        <router-link v-else-if="tab.id === 'usermanual'" :to="{ name: 'UserManual' }" class="usermanual-link"
          style="text-decoration: none;color: white;background-color: #10b981;font-size: 14px;font-weight: 600;border-radius: 4px;margin-top: 15px;
          display: flex;justify-content: center;align-items: center;transition: all 0.3s ease;width: 175px;height: 40px;margin-left: 30px"  >
          User Manual
        </router-link>
      </div>
    ...
    <div v-if="activeTab === 'applications'" class="tab-panel">    
      ...
      <div v-if="setDocFlag==true">
        <label id="lblTooltipTitle" type="text" style="display: flex;margin: auto;justify-content: center;border: solid black 0px;width: 100%;font-size: 20px;font-weight: 600;"></label>
        <div id="lblTooltip" type="text"  style="display: flex;margin: auto;justify-content: center;border: solid black 0px;width: 100%;font-size: 18px;font-weight: 600"></div>
      </div>  
      ...
      <div class="w-full" style="display: inline-flex;margin: auto;justify-content: left;border: solid black 0px;color: white;border-radius: 5px;
        margin-bottom: 5px">
        <input id="divApplicationInfo" @click="toggleSearch()" type="button" class="btn" style="text-align: left;margin-left: 0px;font-size: 14px;font-weight: 600;
      ...
      <div class="w-full" style="display: inline-flex;margin: auto;justify-content: left;border: solid black 0px;color: white;border-radius: 5px;
        margin-bottom: 5px">
        <input id="divApplicationInfo" @click="toggleAppInfo()" type="button" class="btn" style="text-align: left;margin-left: 0px;font-size: 14px;font-weight: 600;
          ;color: white" value="Application Information"/>
      </div>
        
      <!-- Applications Section -->
      <div id="divApps" class="divAppWrapper" style="overflow-x: none;max-height: 300px;min-height: 100px;      
        border: solid black 0px;border-radius: 0px;justify-content: left;margin: 5px;border-radius: 10px;background-color: rgb(255, 255, 255)">   
        <table id="tblMain" style="border: solid black 0px" class="w-full">  
          <tbody>                 
            <tr style="display: inline-flex;margin: auto;border: solid black 0px;height: 40px">      
              <td style="display: inline-flex;margin: auto;border: solid black 0px">                
                <input id="checkSelectAll" @click="selectUnselectAll()" type="checkbox" class="w3-check w-3rem" 
                  style="margin-bottom: 10px;margin-left: 2px;"/>
                <label for="checkSelectAll" style="font-size: 14px;margin-top: 5px;font-weight: 700">{{ selectAll }} All</label>
                <pre><label for="checkSelectAll" style="font-size: 16px;margin-top: 5px;font-weight: 800;color: red;">                                       * Editing is allowed only for the fields marked with an asterisk</label></pre>            
              </td>
            </tr>
          </tbody>
        </table>      
        <div id="divApps" class="divAppWrapper" style="overflow-x: auto;margin: 0px auto;max-height: 250px;min-height: 200px;margin-bottom: 15px;
          border: solid black 0px;border-radius: 0px;justify-content: left;margin-top: 0px;background-color: rgb(255, 255, 255)">   
          <table id="tblMain" class="" style="border: solid black 0px">      
            <thead>
              <tr style="font-size: 14px">                             
                <th id="thSelect" contenteditable="true" style="color: purple;border: solid black 0px;width: 12px;padding-left: 0px;text-align: center;">Select</th>          
                <th id="thBatch" contenteditable="true" style="color: purple;border: solid black 0px;width: 0px;padding-left: 0px;text-align: center;">Batch</th>          
            ...
            <tbody v-if="appCount>0">                          
              <tr v-for="(application,index) in filteredIds" :key="index" :id="'divApps'+index"
                @click="onActive(index,application.Id,application.Guardians[0].LastName,application.Guardians[0].FirstName,
                  application.BatchNum)" @mouseover="setOver(index)" @mouseleave="onLeave()" style="border: solid black 0px;height: 10px;background-color: white">
                <td>                                      
                  ...
                  <input v-else-if="application.Selected=='false'" @click="addToSelected(index)" :id="'chkSelect'+index" 
                    name="inpSelect" type="checkbox" class="w3-check select approw" style="margin-bottom: 13px">
                </td>
                <td>
                  <input :id="'txtBatchNum'+index" :value="application.BatchNum" contenteditable="false" style="font-size: 14px;padding-left: 15px;color: red;font-weight: 700">
                </td>
                ...
                <td :id="'txtId'+index" style="font-size: 14px;padding-left: 0px;color: black">                
                  <input :id="'txtId'+index" :value="application.Id" style="border:solid black 0px;font-size: 14px;height: 10px" readonly> 
                </td>                          
                <td>                 
                ...
                <td v-if="application.Status[0].Checked=='true'">
                  <input :id="'txtMobilStatus0'+index" :value="application.Status[0].MobileStatus" readonly style="border:solid black 0px;font-size: 14px;height: 10px">                                
                </td>          
                <td v-else-if="application.Status[1].Checked=='true'">            
                  <input :id="'txtMobilStatus1'+index" :value="application.Status[1].MobileStatus" readonly style="border:solid black 0px;font-size: 14px;height: 10px">                                
                </td>          
                <td v-else-if="application.Status[2].Checked=='true'">
                  <input :id="'txtMobilStatus2'+index" :value="application.Status[2].MobileStatus" readonly style="border:solid black 0px;font-size: 14px;height: 10px">                                
                </td>
                <td>
                  <input :id="'txtMobilReason0'+index" :value="application.Reasons[0].MobileReason" readonly style="border:solid black 0px;font-size: 14px;height: 10px">                                                            
                </td>
                <td style="font-weight: 700;">
                  <input :id="'txtMobilRow'+index" :value="application.Index" readonly style="border:solid black 0px;font-size: 14px;height: 10px;width:60px;text-align: center;">                                                            
                </td>                            
              </tr>
            </tbody>
            <tbody v-else><tr><td colspan="10" style="padding-left: 10px;font-size: 14px;color: red;font-weight: 600">No matching records found</td></tr></tbody>
          </table>
        </div>    
      </div>     
      ...
      <div v-if="user=='System Administrator'" style="display: flex;margin: auto;border: solid black 0px;justify-content: space-evenly;
        ...
        <input id="inpUpdateRow" name="inpUpdateRow" type="submit" class="w3-btn w3-blue inpUpdateRow" 
          ...
          color: black;width: 300px;border: solid black 0px" value="Update Modified Row">
      </div>
    ...
    <div v-if="this.jsonData" class="hidden" id="divDocumentationWrapper" style="display: flex;margin: auto;
      justify-content: left;border: solid rgb(0, 0, 0) 0px;background-color: rgb(255, 255, 255);overflow: auto;margin-top: 0px;margin-bottom: 12;border-radius: 0px">    
      <div class="w-full" style="display: flex;margin: auto;flex-direction: row;justify-content: center;align-items: center;
        ...
        <input id="divDoc" type="button" class="btn" style="text-align: left;font-size: 14px;font-weight: 600;
   ...
   <div v-if="setDocFlag==true" style="display: block;margin: auto;justify-content: center;border: solid black 0px;width: 100%;margin-top: 10px;margin-bottom: 10px">
      ...
      <div id="divExpected" type="text"  style="display: flex;margin: auto;justify-content: center;border: solid black 0px;width: 100%;font-size: 14px;font-weight: 600">    
        <div v-if="searchtype=='sentTrue'">
          <div style="font-size: 14px;font-weight: 600">Record Count:<label style="margin-left: 10px">2</label></div>
          <div style="font-size: 14px;font-weight: 600">Application Id::<label style="margin-left: 10px">2025952</label><label style="margin-left: 10px">2025975</label></div>                
          <div style="display: flex;margin: auto;width: 250px">
            <div style="display: flex;margin: auto;color: green;width:100px;border: solid black 0px">
              ...
              <input id="inpTestSentYesPassed" name="inpTestSentYes" type="radio" class="w3-check">
            </div>
            <div style="display: flex;margin: auto;color: red;width:100px;border: solid black 0px">
              ...
              <input id="inpTestSentYesFailed" name="inpTestSentYes" type="radio" class="w3-check">
            </div>
          </div>
        </div>      
        <div>
          <div style="font-size: 14px;font-weight: 600;border: solid black 0px;padding: 10px;justify-content: center;align-items: center;text-align: center">Record Count:<br>
            <label style="margin-left: 10px">2</label><br>
            Application Id's:<br><label style="margin-left: 10px">2025951</label>
    ...
    <div v-if="activeTab ==='editLetter'" class="w-full" style="display: flex;margin: auto;flex-direction: row;justify-content: center;
      align-items: center;flex-wrap: wrap;border: solid black 0px;margin-top: 5px">
      <input id="divSearchBy" @click="toggleEditLetter()" type="button" class="btn" style="text-align: left;font-size: 14px;font-weight: 600;
        color: white" value="Eligibility Letter"/>                          
    </div>
    <!-- Letter in HTML Format -->
    <div v-if="activeTab ==='editLetter'" id="divEligibilityLetter" style="display: flex;margin: auto;font-size: 24px;border: solid black 0px;
      padding-top: 20px;justify-content: left;align-items: center;height: 625px" class="divEligibilityLetter">
      <div v-if="jsonData.Applications[activeIndex].Language=='English'" class="" style="display: block;margin: auto;margin-top: 0px;font-size: 24px;
        border: dashed black 1px;padding: 10px;justify-content: center;align-items: center;max-height: 600px;width: 75%;overflow-y: auto">
      ...
      <div v-if="jsonData.Applications[activeIndex].Language=='Spanish'" class="" style="display: block;margin: auto;margin-top: 0px;font-size: 24px;
        border: dashed black 5px;padding: 40px;justify-content: center;align-items: center;width: 25%">        
        <div style="display: flex;margin: auto; justify-content: space-between;border: solid black 0px;width: 100%">      
          <div id="divTitle" class="prop" onkeyup="showProps(this.id,this.innerHTML)" onclick="showProps(this.id,this.innerHTML)"
            style="display: flex;margin: auto; justify-content: center;border: solid black 0px;
      ...
      <div class="" style="display: block;margin: auto;margin-top: 0px;font-size: 24px;border: solid black 0px;margin-left: 0px;
        justify-content: center;align-items: center;width: 25%">                
        <div id="divTools" class="tools col-5" style="margin-left:0px">                    
          <div id="divProps" class="w3-container prop" style="margin-top:0px;padding-right:0px;padding-top: 0px;padding-bottom: 0px;margin-right:5px;
            text-align:right;display:block">
            <table id ="tblProps" style="margin-top: 0px;margin-right:0px;border: solid black 1px;width: 425px;height: 510px;overflow-y: auto;font-weight: 600">                                  
              <tr style="margin-top:10px;margin-left: 0px;border: solid black 0px">
                <td>
    ...
    <div v-if="this.selected.length>0" id="divDisplay" style="display: flex;margin: auto;justify-content: center;margin-top: 0px;border: solid black 0px;position:fixed;
      bottom:0px" class="w-full">    
    ...
    <iframe id="embDoc" allow="fullscreen" :src="selDoc+'#page=1&zoom=180'" style="width: 100%;height: 1100px;overflow-x: hidden;
      overflow-y: auto;border: solid black 0px" 
...
<script>
  ...
  import { Customers } from 'C:/EPISD/customers.json'
  import { Documentation } from 'C:/EPISD/documentation.json'
  ...
  export default {
    ...
    name: "apps",
      ...
      methods: {          
        ...
        addToSelected(index) {   
          ...
          // Wait for Vue to update the DOM before accessing the buttons
          this.$nextTick(() => {
      ...
      async created() {        
        ...
        try {
          ...
          endpoint = config.panels[6].fields[5].value
    ...
    setup() {
      ...
      const tabs = [
        ...
        { id: 'editLetter', name: 'Edit Letter' },
        { id: 'print', name: 'Print Letters' },
        { id: 'show', name: 'Show Letters' },
        { id: 'send', name: 'Send Letters' },
        { id: 'update', name: 'Update Row' },
      ...
      const setOver = (index) => {
        ...
        $("#divApps tbody tr").css("background-color", "white")
        $("#divApps tbody tr").css("border", "solid black 0px")
        $("#divApps"+index).css("background-color", "lightgray")
        $("#divApps"+index).css("border", "solid black 1px")
      }          
...
<style>
  ...
  .searchBy {width: 100%;border: solid rgb(34, 0, 255) 0px;justify-content: center;text-align: center;margin: 20px;font-size: 14px;}
  .filter {width: 13rem!important;justify-content: center!important;margin: 5px; font-size: 14px;} 
  ...
  .filter {width: 105px!important;height: 30px!important;font-size: 14px!important;}
  ...
  @media print { .divNav {display: none!important;  }.eligibility {overflow-y: hidden;border: none!important;width: 1000px!important;height: 100%!important;font-size: 14px!important;}
  .eligibility2 {overflow-y: hidden;border: none!important;width: 1000px!important;height: 100%!important;font-size: 14px!important;}
  ...
  thead th {z-index: 1;position: sticky;top: 0;background-color: rgb(255, 255, 255);padding: 0rem;height: 0px;color: rgb(0, 0, 0);  border: solid rgb(255, 255, 255) 1px;
    padding: 10px;}
  #app {margin-top: 60px;}
  .btn {
